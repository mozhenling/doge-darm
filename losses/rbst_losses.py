...
"""
Source: 
    https://github.com/hitcszx/ALFs/blob/master/losses.py
    https://github.com/HanxunH/Active-Passive-Losses/blob/master/loss.py
    https://github.com/ErikEnglesson/GJS/blob/main/losses.py
"""

eps = 1e-7

##################################
class GLSLoss(torch.nn.Module):
    def __init__(self, num_classes, smooth_rate=None):
        super(GLSLoss, self).__init__()
        self.num_classes = num_classes
        self.smooth_rate = 0.1 if smooth_rate is None else smooth_rate

    def forward(self, logits, labels):
        # (Generalized) Label Smoothing loss
        ...
        # smooth_rate candidates we adopted in the paper: [0.8, 0.6, 0.4, 0.2, 0.0, -0.2, -0.4,
...
class JSDLoss(torch.nn.Module):
    # Jensen-Shannon divergence loss, d_weight is the distribution weight
    # modified from https://github.com/ErikEnglesson/GJS/blob/main/losses.py
    def __init__(self, num_classes, d_weight):
        super(JSDLoss, self).__init__()
        self.num_classes = num_classes
        self.weights = [d_weight, 1-d_weight]
...
class FocalLoss(torch.nn.Module):
    ...
    def __init__(self, num_classes=10, gamma=0.5, alpha=None, size_average=True):
...
class NFocalLoss(torch.nn.Module):
    'NormalizedFocalLoss'
...
class NFLandRCE(torch.nn.Module):
    def __init__(self, alpha=1., beta=1., num_classes=10, gamma=0.5):
        ...
        self.num_classes = num_classes
        self.nfl = NFocalLoss(gamma=gamma, num_classes=num_classes, scale=alpha)
        self.rce = RCELoss(num_classes=num_classes, scale=beta)
...
class NFLandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=10, a=3, q=2):
        ...
        self.num_classes = num_classes
        self.nce = NFocalLoss(num_classes=num_classes, scale=alpha)
        self.agce = AGCELoss(num_classes=num_classes, a=a, q=q, scale=beta)
